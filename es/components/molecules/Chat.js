...
export default class Chat extends Shadow() {
  constructor (options = {}, ...args) {
    ...
    this.eventListener = async event => {
      ...
      console.log('****chat update*****', dataName, event.detail);
      (await event.detail[dataName]()).sort((a, b) => a.timestamp - b.timestamp).forEach((entry, i, chat) => {
        ...
        li.innerHTML = `<chat-a-nick-name class="user" uid='${entry.uid}' nickname="${entry.updatedNickname}"${entry.isSelf ? ' self' : ''}></chat-a-nick-name>
